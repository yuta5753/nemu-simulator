...
(function () {
  ...
  function render(el, state, api) {
    ...
    el.innerHTML = `
      <div class="report-tools no-print"><button type="button" class="sbtn primary" data-action="print">印刷／PDF保存</button><span class="note-p">A4縦・7ページ構成（カテゴリ数により前後します）。印刷ダイアログで「PDFに保存」を選べます。内容は①〜③の最新状態です。</span></div>
      <div class="report">
        ...
        <section class="rpage"><h2>1. 現状サマリー（${U().PERIOD_LABEL(period)}で見た場合）</h2>${D.kpis(st)}${D.table(st)}</section>
        <section class="rpage"><h2>2. 診断（ポートフォリオ・効きどころ）</h2><h3>間口ポートフォリオ</h3>${D.quadrantSvg(pf)}<h3>効きどころ</h3>${D.leverageBlock(lv)}</section>
        <section class="rpage"><h2>2. 診断（つづき：弱点・打ち時・コメント）</h2><h3>弱点候補</h3>${D.weaknessBlock(wk)}<h3>購入までの期間</h3>${D.timingBlock(tm)}<h3>コメント</h3><ul class="cm">${cm.map(c => `<li>${esc(c)}</li>`).join('')}</ul></section>
        <section class="rpage"><h2>3. 目標とギャップ</h2>${P.reverseTable(rv)}</section>
        <section class="rpage"><h2>4. 戦略（${esc(sc.name)}）</h2><p>この配分での売上 ${yen(now.revenue)}（目標到達率 ${reach == null ? '—' : fmt1(reach) + '%'}）</p>
        ...
        <section class="rpage"><h2>5. 前提と計算式</h2><ul class="cm">
          <li>間口＝新規のお客様が最初に買う商品のくくりです。新規獲得人数は1年分を1つの集団として扱い、その集団が12／24／36ヶ月で生む売上を「期間累計売上」と呼びます。</li>
